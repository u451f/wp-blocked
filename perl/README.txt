...
[1mDESCRIPTION[0m
    Minimal URL submit/status implementation of Censorship Monitoring Project
    API
...
[1mMETHODS[0m
  ...
  [1mpush_request()[0m
    ...
    SSL_verify_mode is set to SSL_VERIFY_NONE, so we cannot currently be sure
    of who we're talking to. Results can be retrieved from push_response().
  ...
  [1mget_status()[0m
    Tries to get the status for current URL from network. If this fails with a
    404 status it tries to push the URL to the network first, then retries.
    Result can be retrieved from status_response().

    Returns self, dies on all other errors.

  [1mstatus_Returns self, desponse()[0m
